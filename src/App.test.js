...
const URL = 'https://imr3-react.herokuapp.com/backend'
...
test("Backend is called", async () => {
    render(<App/>);
    await jest.spyOn(global, 'fetch').mockImplementation(setupFetchStub(backendData));

    const res = await fetch(URL)
    const json = await res.json()
    expect(json).toEqual({data: backendData})
